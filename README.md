...
.env.example                                        # deployment settings (location, hostname, …)
scripts/build-dashboard.sh                          # renders the dashboard JSON from .env
grafana/dashboards/weather-forecast.json.template   # committed dashboard source ({{TOKENS}})
grafana/dashboards/weather-forecast.json            # generated by the script (gitignored)
docs/                                               # nginx / NPM / Grafana guides
```

## Build the dashboard

The dashboard's query URLs (location + public host) are rendered from `.env`, so they always
match your deployment:

```bash
cp .env.example .env       # then set LATITUDE / LONGITUDE / PUBLIC_HOST (and FORECAST_DAYS / TIMEZONE)
./scripts/build-dashboard.sh
```

This writes `grafana/dashboards/weather-forecast.json` (gitignored). Import that file into
Grafana — see [docs/grafana-setup.md](docs/grafana-setup.md).

## Changing the location

The cache is location-agnostic — it caches whatever `/v1/*` URL is requested. To change the
forecast point, edit `LATITUDE`/`LONGITUDE` in `.env` and re-run `./scripts/build-dashboard.sh`,
then re-import the JSON. Adding more locations = more Grafana queries; nginx caches each unique
URL automatically, no config change needed.
